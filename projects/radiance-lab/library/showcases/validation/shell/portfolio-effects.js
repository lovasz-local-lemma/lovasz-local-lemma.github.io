...
(() => {
  ...
  const ready = () => {
    ...
    if (rail) {
      ...
      let geometry = null, floors = [], crossedIndex = -1;
      ...
      const renderCar = () => {
        rail.style.setProperty('--rail-y', `${position.toFixed(2)}px`);
        rail.style.setProperty('--rail-height', `${carHeight}px`);
        if (geometry) {
          const center = position + carHeight / 2 - rail.scrollTop;
          aura.style.transform = `translate3d(${geometry.left + geometry.width / 2 - 80}px, ${geometry.top + center - 66}px, 0)`;
          aura.dataset.visible = String(center > 0 && center < geometry.height);
        }
      };
      const arrive = () => {
      ...
      const updateRail = () => {
        ...
        if (!Number.isFinite(position) || motion.matches) {
          ...
          rail.dataset.direction = destination > position ? 'down' : 'up';
          rail.dataset.moving = 'true';
          links.forEach(link => link.classList.remove('is-lit'));
