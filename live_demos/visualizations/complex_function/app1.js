...
class ComplexFunctionVisualizer {
    ...
    init() {
        ...
        this.state = {
            function: 'cbrt',
            vizMode: null, // Start with no default - user must choose
            surfaceMode: 'surface', // 'plane' or 'surface' - controls whether to show flat plane or 3D surface
            curvedArrows: true,
            thickArrows: true,
            arrowOpacity: 0.7,
            gridDensity: 10,
    ...
    // Compute derivative for a specific branch of a multi-valued function
    computeDerivativeForBranch(z, branch, h = 0.001) {
        const allValues = this.getAllValues(z);
        const allValues_h = this.getAllValues({ re: z.re + h, im: z.im });
        
        const fz = allValues[branch];
        const fz_h = allValues_h[branch];
    ...
    createToPlanes() {
        ...
        // Don't create any planes if no vizMode selected yet
        if (!this.state.vizMode) {
            return;
        }
        
        // For Riemann surface modes, don't create toPlanes - surface created separately
        if (this.state.vizMode === 'riemann_re' || this.state.vizMode === 'riemann_im') {
            return;
        }
        ...
        // For magnitude/argument modes, create texture-based planes
        if (this.state.vizMode === 'magnitude' || this.state.vizMode === 'argument') {
            ...
            plane.position.z = this.state.separation / 2;
            this.scene.add(plane);
            this.toPlanes.push(plane);
            return; // Texture will be set by createMagnitudeVisualization/createArgumentVisualization
        }
    ...
    addGridLines() {
        ...
        const createTubeGrid = (zPos) => {
            ...
            const cornerMaterial = gridMaterial;
            const corners = [
                [-halfSize, -halfSize],
                [-halfSize, halfSize],
                [halfSize, -halfSize],
            ...
            corners.forEach(([cx, cy]) => {
                ...
                const sphere = new THREE.Mesh(sphereGeo, cornerMaterial);
    ...
    updateVisualization() {
        ...
        if (this.derivativeMesh) {
            ...
            if (this.derivativeMesh.geometry) this.derivativeMesh.geometry.dispose();
            if (this.derivativeMesh.material) this.derivativeMesh.material.dispose();
            this.derivativeMesh = null;
        }
        ...
        // Show/hide notification if no vizMode selected
        this.updateNotification();
        
        // If no vizMode selected, don't create any visualization
        if (!this.state.vizMode) {
            return;
        }
        
        // Only create toPlanes for modes that need flat colored planes
        // magnitude and argument create their own surfaces
        if (this.state.vizMode !== 'magnitude' && this.state.vizMode !== 'argument') {
            this.createToPlanes();
        }
        ...
        switch(this.state.vizMode) {
            // case 'color':
            //     // Update toPlane colors if using derivative
            //     if (this.state.funcType === 'derivative') {
            //         this.updateToPlaneWithDerivative();
            //     }
            //     break;
            // case 'arrows':
            ...
            //     break;
            case 'riemann_re':
                this.createRiemannSurface('real');
                break;
            case 'riemann_im':
                this.createRiemannSurface('imag');
                break;
            case 'magnitude':
                this.createMagnitudeVisualization();
                break;
            case 'argument':
                this.createArgumentVisualization();
                break;
            // case 'distortion':
        ...
        // For surface modes (Riemann, magnitude, argument), control derivativeMesh visibility with showToPlane toggle
        if (this.derivativeMesh && (this.state.vizMode === 'riemann_re' || this.state.vizMode === 'riemann_im' || this.state.vizMode === 'magnitude' || this.state.vizMode === 'argument')) {
            this.derivativeMesh.visible = this.state.showToPlane;
        }
    ...
    createHighlightVisualization(clickPoint) {
        ...
        // Add surface intersection markers if in surface mode
        if (this.state.vizMode === 'riemann_re' || this.state.vizMode === 'riemann_im' || 
            this.state.vizMode === 'magnitude' || this.state.vizMode === 'argument') {
            this.addSurfaceIntersectionMarkers(clickPoint, size, segments);
        }
    ...
    // Add glowing markers where vertical tube intersects the surface
    addSurfaceIntersectionMarkers(center, radius, segments) {
        const shapePoints = this.getShapePoints(center.x, center.y, radius, segments * 3, this.state.morphTime);
        
        // Determine number of branches
        const testZ = { re: 0, im: 0 };
        let numBranches = 1;
        if (this.state.showMultiValue) {
            numBranches = this.getAllValues(testZ).length;
        }
        
        // Create intersection markers for each branch
        for (let branch = 0; branch < numBranches; branch++) {
            const curvePoints = [];
            const curveColors = [];
            
            // Sample points along the vertical tube perimeter to find surface intersections
            for (let i = 0; i < segments * 3; i++) {
                const point = shapePoints[i];
                const z = { re: point.x, im: point.y };
                
                const funcType = this.state.funcType || 'function';
                let result;
                if (funcType === 'derivative') {
                    if (this.state.showMultiValue && numBranches > 1) {
                        result = this.computeDerivativeForBranch(z, branch);
                    } else {
                        result = this.computeDerivative(z);
                    }
                } else {
                    if (this.state.showMultiValue && numBranches > 1) {
                        const allValues = this.getAllValues(z);
                        result = allValues[branch];
                    } else {
                        result = this.applyFunction(z);
                    }
                }
                
                // Calculate surface height based on current viz mode
                let surfaceHeight = 0;
                if (this.state.vizMode === 'riemann_re') {
                    surfaceHeight = result.re * this.state.heightScale;
                } else if (this.state.vizMode === 'riemann_im') {
                    surfaceHeight = -result.im * this.state.heightScale;
                } else if (this.state.vizMode === 'magnitude') {
                    const mag = Math.sqrt(result.re * result.re + result.im * result.im);
                    surfaceHeight = mag * this.state.heightScale;
                } else if (this.state.vizMode === 'argument') {
                    const angle = Math.atan2(result.im, result.re);
                    surfaceHeight = angle * this.state.heightScale;
                }
                
                // Position on the VERTICAL TUBE (not mapped point) - straight up at (point.x, point.y)
                const position = new THREE.Vector3(point.x, point.y, this.state.separation / 2 + surfaceHeight);
                curvePoints.push(position);
                
                // Color gradient
                const hue = (i / (segments * 3) + branch / numBranches) % 1.0;
                const color = new THREE.Color().setHSL(hue, 1.0, 0.6);
                curveColors.push(color.r, color.g, color.b);
                
                // Create smaller glowing markers
                ...
                this.state.highlightMesh.add(marker);
            }
            
            // Connect markers with a closed curve
            ...
            const curveLine = new THREE.Line(curveGeometry, new THREE.LineBasicMaterial({ 
                vertexColors: true,
                linewidth: 3,
                transparent: true,
                opacity: 0.9
            }));
            this.state.highlightMesh.add(curveLine);
        }
    }
    ...
    createPipe(center, radius, segments) {
        // For surface modes: create straight vertical extrusion (upright cylinder)
        // This is just a visual guide showing the region being extruded upward
        
        const heightSteps = 30;
        const maxHeight = 5; // Maximum height for the vertical extrusion
        
        // Get shape points at the base
        const shapePoints = this.getShapePoints(center.x, center.y, radius, segments, this.state.morphTime);
        
        // Build vertices and faces for the vertical extrusion
        const vertices = [];
        const colors = [];
        const faces = [];
        
        // Create vertical rings at each height level
        for (let h = 0; h <= heightSteps; h++) {
            const t = h / heightSteps;
            const z = -this.state.separation / 2 + t * maxHeight;
            
            for (let i = 0; i <= segments; i++) {
                const point = shapePoints[i];
                
                // Keep x,y constant (straight up), only vary z
                vertices.push(point.x, point.y, z);
                
                // Dim gradient color - darker at bottom, lighter at top
                const hue = i / segments;
                const lightness = 0.3 + t * 0.2; // Dim color
                const color = new THREE.Color().setHSL(hue, 0.6, lightness);
                colors.push(color.r, color.g, color.b);
            }
        }
        
        // Create faces connecting the vertical rings
        for (let h = 0; h < heightSteps; h++) {
            for (let i = 0; i < segments; i++) {
                const a = h * (segments + 1) + i;
                const b = h * (segments + 1) + i + 1;
                const c = (h + 1) * (segments + 1) + i + 1;
                const d = (h + 1) * (segments + 1) + i;
        ...
        // Semi-transparent material with lower opacity
        const material = new THREE.MeshPhongMaterial({
            vertexColors: true,
            transparent: true,
            opacity: 0.2, // More transparent than before
            side: THREE.DoubleSide,
            shininess: 10,
            flatShading: false
        });
    ...
    // Create Riemann surface visualization (height = Re(f) or Im(f))
    createRiemannSurface(component) {
        ...
        // Create group to hold all branches
        this.derivativeMesh = new THREE.Group();
        ...
        for (let branch = 0; branch < numBranches; branch++) {
            ...
            for (let i = 0; i < resolution; i++) {
                for (let j = 0; j < resolution; j++) {
                    ...
                    const x = (j / (resolution - 1) - 0.5) * size;
                    const y = (i / (resolution - 1) - 0.5) * size;
                    
                    const z = { re: x, im: y };
                    
                    // Get function or derivative based on funcType
                    let result;
                    if (this.state.funcType === 'derivative') {
                        if (this.state.showMultiValue && numBranches > 1) {
                            result = this.computeDerivativeForBranch(z, branch);
                    ...
                    // Set height based on component and surfaceMode
                    if (this.state.surfaceMode === 'surface') {
                        const height = component === 'real' ? result.re : result.im;
                        positions[index + 2] = height * this.state.heightScale;
                    } else {
                        positions[index + 2] = 0; // Flat plane
                    }
                    
                    // Color by argument
                    const hue = ((Math.atan2(result.im, result.re) + Math.PI) / (2 * Math.PI)) * 360;
            ...
            // All branches overlap at same position
            mesh.position.z = this.state.separation / 2;
            
            this.derivativeMesh.add(mesh);
        }
        
        this.scene.add(this.derivativeMesh);
    }
    
    // Create magnitude surface visualization (height = |f(z)|)
    createMagnitudeVisualization() {
        const resolution = 100;
        const size = 6;
        ...
        for (let branch = 0; branch < numBranches; branch++) {
            ...
            for (let i = 0; i < resolution; i++) {
                for (let j = 0; j < resolution; j++) {
                    ...
                    const x = (j / (resolution - 1) - 0.5) * size;
                    const y = (0.5 - i / (resolution - 1)) * size;  // Flip y to match Three.js coordinate system
                    ...
                    // Set height based on magnitude and surfaceMode
                    const magnitude = Math.sqrt(result.re * result.re + result.im * result.im);
                    if (this.state.surfaceMode === 'surface') {
                        positions[index + 2] = magnitude * this.state.heightScale;
                    } else {
                        positions[index + 2] = 0; // Flat plane
                    }
                    
                    // Color by magnitude (grayscale gradient)
                    const brightness = Math.min(1.0, magnitude * 0.3);
    ...
    // Create argument visualization as a 3D surface with height = arg(f(z))
    createArgumentVisualization() {
        const resolution = 100;
        const size = 6;
        ...
        for (let branch = 0; branch < numBranches; branch++) {
            ...
            for (let i = 0; i < resolution; i++) {
                for (let j = 0; j < resolution; j++) {
                    ...
                    // Set height based on argument (angle) and surfaceMode
                    const angle = Math.atan2(result.im, result.re);
                    if (this.state.surfaceMode === 'surface') {
                        positions[index + 2] = angle * this.state.heightScale;
                    } else {
                        positions[index + 2] = 0; // Flat plane
                    }
                    
                    // Color by argument (hue wheel)
    ...
    // Update toPlane with derivative colors for color mode
    updateToPlaneWithDerivative() {
        this.toPlanes.forEach(plane => {
            ...
            for (let i = 0; i < positions.length; i += 3) {
                ...
                const y = positions[i + 1];
                const z = { re: x, im: y };
                const result = this.computeDerivative(z);
    ...
    // Update notification message for when no vizMode is selected
    updateNotification() {
        let notification = document.getElementById('vizModeNotification');
        
        if (!this.state.vizMode) {
            // Create notification if it doesn't exist
            if (!notification) {
                notification = document.createElement('div');
                notification.id = 'vizModeNotification';
                notification.style.position = 'absolute';
                notification.style.top = '50%';
                notification.style.left = '50%';
                notification.style.transform = 'translate(-50%, -50%)';
                notification.style.padding = '30px 50px';
                notification.style.background = 'rgba(52, 152, 219, 0.25)';
                notification.style.border = '2px solid rgba(52, 152, 219, 0.8)';
                notification.style.borderRadius = '10px';
                notification.style.color = '#F4D03F';
                notification.style.fontSize = '28px';
                notification.style.fontWeight = 'bold';
                notification.style.textAlign = 'center';
                notification.style.zIndex = '1000';
                notification.style.pointerEvents = 'none';
                notification.style.textShadow = '0 0 4px rgba(244,208,63,0.8), 0 0 10px rgba(244,208,63,0.6), 0 2px 4px rgba(0,0,0,0.5)';
                notification.textContent = 'Choose a Component to Show →';
                document.getElementById('container').appendChild(notification);
            }
            notification.style.display = 'block';
        } else {
            // Hide notification if vizMode is selected
            if (notification) {
    ...
    setupEventListeners() {
        ...
        document.querySelectorAll('input[name="funcType"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                this.state.funcType = e.target.value;
                this.updateVisualization();
            });
        });
        ...
        document.querySelectorAll('input[name="vizMode"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                ...
                document.getElementById('riemannControls').style.display = 
                    (e.target.value === 'riemann_re' || e.target.value === 'riemann_im') ? 'block' : 'none';
                document.getElementById('distortionControls').style.display = 
                    e.target.value === 'distortion' ? 'block' : 'none';
                
                this.updateVisualization();
            });
        });
        
        document.querySelectorAll('input[name="surfaceMode"]').forEach(radio => {
            radio.addEventListener('change', (e) => {
                this.state.surfaceMode = e.target.value;
                this.updateVisualization();
            });
        });
        ...
        document.getElementById('riemannHeightScale').addEventListener('input', (e) => {
            this.state.heightScale = parseFloat(e.target.value);
            document.getElementById('riemannHeightScaleValue').textContent = e.target.value;
            this.updateVisualization();
        });
