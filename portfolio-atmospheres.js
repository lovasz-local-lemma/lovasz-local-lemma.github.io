...
(() => {
  ...
  function create(canvas, motion, onChange) {
    ...
    // Reuse vertices: these Canvas2D paths are consumed synchronously. The
    // animated contour preset otherwise allocated over 10,000 tiny arrays per
    // paint despite keeping the same finite sample budget. Coordinates, draw
    // order, colors and sample counts remain identical.
    const pointBuffers = [[], []];
    const pointsFor = (count, slot = 0) => pointBuffers[slot][count] ||
      (pointBuffers[slot][count] = Array.from({ length: count }, () => [0, 0]));
    const pointAt = (points, index, x, y) => { points[index][0] = x; points[index][1] = y; };
    ...
    function drawCaustics(ctx, w, h, t) {
      ...
      for (let family = 0; family < 2; family++) {
        for (let band = 0; band < bands; band++) {
          const q = band / (bands - 1), points = pointsFor(samples + 1);
          for (let j = 0; j <= samples; j++) {
            ...
            pointAt(points, j, x, family ? h-y+h*.07 : y);
          }
    ...
    function drawOrrery(ctx, w, h, t) {
      ...
      for (let ring=0; ring<7; ring++) {
        const r=base*(.32+ring*.145), points=pointsFor(121);
        const tilt=.54+.065*ring;
        for(let j=0;j<=120;j++) {
          const a=j/120*TAU;
          pointAt(points,j,Math.cos(a)*r,Math.sin(a)*r*tilt);
        }
      ...
      const left=pointsFor(151);
      for(let i=0;i<=150;i++) {const a=i/150*TAU;pointAt(left,i,w*.07+Math.cos(a)*base*.63,h*.23+Math.sin(a)*base*.63);}
    ...
    function drawAurora(ctx,w,h,t) {
      ...
      for(let band=0;band<bands;band++) {
        const q=band/(bands-1), top=pointsFor(samples+1),bottom=pointsFor(samples+1,1);
        for(let j=0;j<=samples;j++) {
          ...
          pointAt(top,j,x,y);pointAt(bottom,j,x,y+6+q*15);
        }
        trace(ctx,top);
        for(let j=bottom.length-1;j>=0;j--)ctx.lineTo(bottom[j][0],bottom[j][1]);
        ctx.closePath();
      ...
      const horizon=pointsFor(81);
      for(let i=0;i<=80;i++){const u=i/80;pointAt(horizon,i,u*w,h*(.87+.025*Math.sin(u*10+t*.08)));}
    ...
    function drawProjectStudy(ctx,w,h,t,kind){
      ...
      for(let side=0;side<2;side++){
        ...
        if(kind==='signals'){
          ...
          for(let n=1;n<=9;n++){
            const points=pointsFor(81);for(let j=0;j<=80;j++){const q=j/80;pointAt(points,j,x0+q*span,h*(.08+n*.09)+Math.sin(q*Math.PI*(2+n%4))*Math.cos(t*.55+n)*h*.05);}
          ...
          for(let i=0;i<5;i++){
            const points=pointsFor(56),cy=h*(.12+i*.17),cx=x0+span*(.2+(i%3)*.26);
            for(let j=0;j<=55;j++){const x=x0+j/55*span;pointAt(points,j,x,cy+(x-cx)**2/(span*.65));}
